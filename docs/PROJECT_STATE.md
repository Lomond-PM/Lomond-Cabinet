...
**0.3.9 — Streaming Response & Reasoning Surface: COMPLETE / SEALED / merged into dev**, PR #182, merge commit `91005f2`. **0.3.10 — Context Architecture: IN PROGRESS**; A0/A1/A2 and A3a/A3b complete; A4a execution/Verify evidence audit and projection contract complete; A4b implemented / offline integration PASS / real AE acceptance pending; A5–A6 not started.

[A4a Verified Trajectory Source & Projection Contract](design/vela-verified-trajectory-0.3.10-a4a.md) defines separate intent/attempt, mutation, commit, verification and completion dimensions. [A4b implementation evidence and pending AE matrix](reports/vela-0.3.10-a4b-verified-trajectory.md) records immutable bounded projection, canonical source hooks before reduction, and AgentRuntimeOwner active/last-terminal retention. Cancellation seals available evidence without late enrichment; dispose/reload clears it. Existing Session committed:true remains unchanged and can include already-satisfied. No trajectory enters Provider input or Authority; A5 owns future selection. A4b has 34 focused offline suites / 4,022 assertions PASS, including 14 pre-A4b behavior comparisons. All six targeted real AE projection cases remain PENDING; A4b is not closed.

[0.3.10-A1 Context Taxonomy, Ownership & Lifecycle Contract](design/vela-context-architecture-0.3.10-a1.md) records the design baseline and deferred decisions. `currentContext` remains a compatibility/read-only Observation projection with unchanged API/behavior. [A2 implementation evidence](reports/vela-0.3.10-a2-context-evidence.md) describes the opt-in local Controller/Adapter input snapshot and stateless Transport serialization projection. Current messages, captures, admission and execution semantics remain unchanged. No history selection, persistent trajectory store or multi-conversation runtime is implemented.
