...
**0.3.9 — Streaming Response & Reasoning Surface: COMPLETE / SEALED / merged into dev**, PR #182, merge commit `91005f2`. **0.3.10 — Context Architecture: IN PROGRESS**; A0/A1 complete, A2 evidence seam merged into dev, A3a capacity/budget policy recorded; A3b enforcement and A4–A6 not started.
...
[A3a Provider Capacity Source & Context Budget Policy Decision](design/vela-provider-capacity-budget-0.3.10-a3a.md) defines future bounded current-shape compatibility for unknown capacity/cost/reserve and conditional numeric fit checks. Current readiness contextLength is advisory, not approved invocation-bound capacity. No enforcement, tokenizer, discovery rewiring or generation tuning was implemented. Next focused slice: A3b bounded policy/enforcement under this contract; empirical instance-routing and near-capacity reserve semantics remain explicit requirements.
