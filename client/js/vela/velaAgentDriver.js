(function (root, factory) {
    ...
    "use strict";
    var MODULE_REVISION = "vela-agent-driver-0.3.7-a1-v1";
    ...
    function createAgentDriver(options) {
        ...
        function snapshot() { return Object.freeze({ state: state, objectiveId: active ? active.objectiveId : null, taskId: active ? active.taskId : null, taskPlan: active ? active.taskPlan : null, turn: active ? active.turn : null, terminal: active ? active.terminal : null, counters: Object.freeze(active ? { observations: active.observations, reasoningTurns: active.reasoningTurns, actions: active.actions, replans: 0 } : { observations: 0, reasoningTurns: 0, actions: 0, replans: 0 }), disposed: disposed }); }
        ...
        function transition(next) { var legal = { idle: ["observing"], observing: ["reasoning", "terminal"], reasoning: ["awaiting-outcome", "terminal"], "awaiting-outcome": ["verifying", "terminal"], verifying: ["terminal"] }; if (!legal[state] || legal[state].indexOf(next) === -1) { throw error(ERROR_CODES.AGENT_DRIVER_ILLEGAL_TRANSITION); } state = next; notify(); }
        function event(kind, payload) { append({ kind: kind, requestId: active.objectiveId, payload: payload || {} }); }
        function terminal(outcome, code) { if (!active || active.terminal) { return snapshot(); } active.terminal = Object.freeze({ outcome: outcome, code: code || null }); transition("terminal"); event(outcome === "completed" ? "task/completed" : outcome === "cancelled" ? "task/cancelled" : "task/blocked", { taskId: active.taskId, taskPlanId: active.taskPlan ? active.taskPlan.planId : null, code: code || null }); return snapshot(); }
        ...
        function startObjective(input) {
            ...
            active = { objectiveId: "objective_agent_" + serial, taskId: "agent_task_" + serial, taskPlan: null, intent: null, turn: beginTurn(), terminal: null, observations: 0, reasoningTurns: 0, actions: 0, committed: false };
            ...
            return Promise.resolve(observe()).then(function () {
                ...
                if (!current(captured) || !outcome || outcome.state === "review-required") { return outcome && outcome.state === "review-required" ? terminal("blocked", "REVIEW_REQUIRED") : snapshot(); }
