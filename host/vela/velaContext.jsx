...
(function () {
    ...
    function assertCommittedPropertyTarget(target) {
        assertKeys(target, ["itemId", "nativeLayerId", "layerIndex", "propertyPath", "propertyMatchName"]);
        assertFiniteNumber(target.itemId, true, 1, MAX_NUMBER_ABS);
        assertFiniteNumber(target.nativeLayerId, true, 1, MAX_NUMBER_ABS);
        assertFiniteNumber(target.layerIndex, true, 1, MAX_NUMBER_ABS);
        assertPropertyPath(target.propertyPath);
        if (typeof target.propertyMatchName !== "string" || !target.propertyMatchName.length || json.utf8ByteLength(target.propertyMatchName) > 56 || target.propertyPath[target.propertyPath.length - 2] !== target.propertyMatchName) {
            fail("HOST_CONTEXT_REQUEST_INVALID");
        }
    }
    ...
    function validateRequest(value) {
        ...
        if (operation === "observeCommittedPropertyValue") {
            if (value.scope.purpose !== "verification") { fail("HOST_CONTEXT_REQUEST_INVALID"); }
            assertHostAuthority(value.scope.expectedHostInstanceId, value.scope.expectedHostReloadEpoch);
            assertFiniteNumber(value.scope.expectedProjectGeneration, true, 1, MAX_NUMBER_ABS);
    ...
    function observeCommittedPropertyValue(request) {
        var project;
        var item = null;
        ...
        observeProject(project);
        if (!project || request.scope.expectedHostInstanceId !== hostInstanceId || request.scope.expectedHostReloadEpoch !== hostReloadEpoch || request.scope.expectedProjectGeneration !== projectGeneration) {
            fail("HOST_CONTEXT_AUTHORITY_MISMATCH");
        }
        try {
            ...
            if (!item) {
                for (i = 1; i <= project.numItems; i++) {
                    if (project.item(i) && project.item(i).id === target.itemId) { item = project.item(i); break; }
                }
            }
        ...
        terminal = resolvePropertyPath(layer, target.propertyPath);
        if (!terminal || terminal.matchName !== target.propertyMatchName) { fail("HOST_CONTEXT_TARGET_NOT_FOUND"); }
        normalizedValue = readPropertyValue(terminal);
        return {
            hostInstanceId: hostInstanceId,
            hostReloadEpoch: hostReloadEpoch,
