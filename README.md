...
当前 Vela：**0.3.11 — Multi-conversation Foundation，COMPLETE / SEALED**；**0.3.12-B1 / G-02：TARGETED_ACCEPTED / READY FOR COMMIT / PR**，见 [G-02报告](docs/reports/vela-0.3.12-b1-g02-layer-name-unicode.md)；A11含F1已通过PR #203合并dev，保持TARGETED_ACCEPTED及覆盖限制，见 [A11 报告](docs/reports/vela-0.3.12-b1-a11-host-json-serialization.md)；A01 已通过 PR #202 合并 dev，保持 TARGETED_ACCEPTED 及覆盖限制，尚未 CLOSED。功能里程碑与包 metadata 独立。见 [唯一当前排期](docs/VELA_ROADMAP.md)、[已实现事实](docs/PROJECT_STATE.md) 和 [0.3.11 封存证据](docs/reports/vela-0.3.11-integrated-acceptance.md)。
