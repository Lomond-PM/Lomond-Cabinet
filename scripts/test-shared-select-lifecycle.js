...
component.viewport.dispatch("wheel", { deltaY: 80 });
assert(component.root.classList.contains("is-open"), "internal wheel input does not close the shared popup");
doc.dispatch("scroll", { target: component.viewport });
assert(component.root.classList.contains("is-open"), "capture-phase internal viewport scroll does not close the shared popup");
doc.dispatch("scroll", { target: component.viewport.children[0] });
assert(component.root.classList.contains("is-open"), "scroll originating inside a popup option remains popup-owned");
doc.dispatch("scroll", { target: doc.body });
assert(!component.root.classList.contains("is-open"), "external document or ancestor scroll closes the shared popup");
component.open(); win.dispatch("resize", { target: win });
assert(!component.root.classList.contains("is-open"), "window resize closes the shared popup");
component.open();
...
let consumed = 0; let stopped = 0;
component.open(); component.trigger.dispatch("keydown", { keyCode: 27, preventDefault() { consumed++; }, stopPropagation() { stopped++; } });
assert.strictEqual(consumed, 1); assert.strictEqual(stopped, 1);
component.trigger.dispatch("keydown", { keyCode: 27, preventDefault() { consumed++; }, stopPropagation() { stopped++; } });
assert.strictEqual(consumed, 1, "closed menu leaves Escape for the outer owner");
...
const secondMount = doc.createElement("div");
doc.body.appendChild(secondMount);
const secondSelect = CoreUI.createSelect({ document: doc, id: "secondaryMode" });
const secondOption = doc.createElement("option"); secondOption.value = "next"; secondOption.textContent = "NEXT"; secondOption.selected = true; secondSelect.appendChild(secondOption); secondMount.appendChild(secondSelect);
const secondComponent = CoreUI.enhanceSelect({ document: doc, select: secondSelect });
component.open(); secondComponent.open();
assert(!component.root.classList.contains("is-open") && secondComponent.root.classList.contains("is-open"), "single-active coordination closes the previous shared Select");
