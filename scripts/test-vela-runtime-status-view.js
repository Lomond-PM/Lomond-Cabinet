...
function run() {
    ...
    const context = {
        ...
        __testController: Object.freeze({ getStatus() { return Object.freeze(current); } }),
    ...
    const allowed = ["schemaRevision", "diagnosticOnly", "state", "initialized", "suspended", "disposed", "loaderState", "moduleRevision", "hostAdapterRevision", "lastErrorCode", "statusRevision"];
    ...
    check(context.VelaRuntimeStatusView.state === "ready" && context.VelaRuntimeStatusView.initialized === true && context.VelaRuntimeStatusView.hostAdapterRevision === "vela-context-host-v4", "Status view reflects the ready runtime state without exposing the controller.");
