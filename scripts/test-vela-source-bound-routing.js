...
function synthetic() {
 ...
 const runtime={getStatus:()=>({state:'ready'}),getProviderSurfaceState(){state.reads++;return state.provider;},getConfirmationSurfaceState:()=>({...state.confirmation,canApprove:true}),subscribePresentationEvents(fn){listeners.push(fn);return {unsubscribe(){const i=listeners.indexOf(fn);if(i>=0)listeners.splice(i,1);}};},approveActiveCandidate(){state.approve++;return operation.promise;},rejectActiveCandidate(){state.reject++;return Promise.resolve();},getProviderSelectionEvidence:()=>state};
