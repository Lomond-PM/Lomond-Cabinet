...
function run() {
    ...
    check(/function reportVelaSurfaceInitializationError\(\)[\s\S]*SURFACE_BOOTSTRAP_UNAVAILABLE/.test(mainSource) && mainSource.includes("return composition.createRecord()") && mainSource.includes("velaConversationComposition.select(record)") && fs.readFileSync(path.join(VELA, "velaConversationComposition.js"), "utf8").includes("candidate.runtime.initialize()") && fs.readFileSync(path.join(VELA, "velaConversationComposition.js"), "utf8").includes("candidate.runtime.getStatus().state !== \"ready\""), "Runtime candidates commit only after initialization while Runtime and Surface bootstrap failures retain distinct error boundaries.");
