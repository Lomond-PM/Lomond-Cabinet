...
async function run() {
    ...
    exhausted.attachRuntimePort({ reason() { exhaustedReason += 1; return Promise.resolve({ capabilityId: "set-opacity-v1", params: { opacity: 47 } }); }, submitIntent() { exhaustedSubmit += 1; return Promise.resolve({ state: "review-required", committed: false, code: "REVIEW_REQUIRED", reviewCorrelation: "exhausted_correlation_" + exhaustedSubmit }); }, continueApprovedReview() { return Promise.resolve({ state: "blocked", code: "CONTEXT_STALE", committed: false, observation: { targetAvailable: true, targetClass: "layer-opacity", observedOpacityDigest: "sha256:same" } }); }, verifyCommittedAction() { throw new Error("unreachable"); }, verifyAction() { throw new Error("unreachable"); }, cancel() { return true; } });
    ...
    transitionCancel.attachRuntimePort({ reason() { transitionReason += 1; return Promise.resolve({ capabilityId: "set-opacity-v1", params: { opacity: 47 } }); }, submitIntent() { return Promise.resolve({ state: "review-required", committed: false, code: "REVIEW_REQUIRED", reviewCorrelation: "cancel_transition_review" }); }, continueApprovedReview() { return Promise.resolve({ state: "blocked", code: "CONTEXT_STALE", committed: false, observation: { targetAvailable: true, targetClass: "layer-opacity", observedOpacityDigest: "sha256:cancel" } }); }, verifyCommittedAction() { throw new Error("unreachable"); }, verifyAction() { throw new Error("unreachable"); }, cancel() { return true; } });
    ...
    const cancellingPort = { reason() { throw new Error("unreachable"); }, submitIntent() { throw new Error("unreachable"); }, continueApprovedReview() { throw new Error("unreachable"); }, verifyCommittedAction() { throw new Error("unreachable"); }, verifyAction() { throw new Error("unreachable"); }, cancel() { return true; } };
