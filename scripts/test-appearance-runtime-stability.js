...
check(/changed = CoreAppearance && CoreAppearance\.commit\(parameter\.id, value\);[\s\S]*changed\.accepted && parameter\.persistence === "settings"\) changed\.persisted = saveSettings\(\) === true/.test(main), "settings authority persists only after successful resolver commit");
