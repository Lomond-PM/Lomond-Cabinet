...
assert.ok(/\.app-shell \*::-webkit-scrollbar[\s\S]*track[\s\S]*thumb[\s\S]*corner/.test(css), "application scope owns the full native WebKit presentation contract");
assert.ok(/html,\s*body,\s*\.app-shell,\s*\.app-shell \*\s*\{[^}]*scrollbar-color:[^}]*scrollbar-width:\s*thin/.test(css), "application native scrollbars receive canonical presentation without consumer opt-in");
