...
function run() {
    ...
    check(/openSettings: openVelaSettingsPanel/.test(mainSource) && /function openVelaSettingsPanel\(launchSource\)[\s\S]*SystemRouter\.open\("settings", "root", source\)[\s\S]*openSettingsPanel\("vela", source\)/.test(mainSource), "Surface Settings uses the controlled transitional Vela category entry point.");
    check(/function openSettingsPanel\(focusSectionId, launchSource\)[\s\S]*view\.classList\.contains\("is-open"\)[\s\S]*focusPendingSettingsSection/.test(openSettings), "An already-open Settings panel repositions Vela without rebuilding Settings content.");
    ...
    check(/pages: \["root", "appearance"\]/.test(mainSource) && /createSettingsCategory\("vela"/.test(mainSource) && !/data-settings-destination/.test(mainSource), "Vela remains a transitional root disclosure rather than a permanent Global Settings route.");
