...
function run() {
    ...
    check(/velaProviderModel: VelaProviderModel/.test(collectSettings) && /velaProviderEndpoint: VelaProviderEndpoint/.test(collectSettings) && /velaExperimentalAcknowledged: VelaExperimentalAcknowledged === true/.test(collectSettings) && !/experimentalEnabled|readiness/.test(collectSettings), "Settings persists configuration and explicit opt-in, never Provider session readiness or authority.");
    check(/VelaExperimentalAcknowledged = false/.test(mainSource) && /velaExperimentalAcknowledged:/.test(collectSettings), "A fresh install defaults opt-in off while an explicit stored opt-in may be restored.");
    check(/VelaProviderModel = typeof data\.velaProviderModel[\s\S]*VelaProviderEndpoint = typeof data\.velaProviderEndpoint[\s\S]*VelaExperimentalAcknowledged = data\.velaExperimentalAcknowledged === true[\s\S]*byId\("velaProviderModel"\)\.value = VelaProviderModel[\s\S]*byId\("velaProviderEndpoint"\)\.value = VelaProviderEndpoint/.test(sliceFunction("applySettings", "loadPersistentState")), "Stored endpoint, model and opt-in repopulate Settings without restoring Provider session readiness.");
