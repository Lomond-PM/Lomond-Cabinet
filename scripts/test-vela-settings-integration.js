...
function run() {
    ...
    check(/ActivationPolicy:\s*window\.VelaActivationPolicy/.test(mainSource) && /provider: sourcePort.provider/.test(mainSource) && /checkProviderReadiness/.test(fs.readFileSync(path.join(__dirname, "..", "client", "js", "vela", "velaConversationOwnership.js"), "utf8")) && !/productionEnabled\s*=/.test(renderVela), "Production activation comes only from the trusted policy while readiness remains an explicit Runtime action outside Settings.");
