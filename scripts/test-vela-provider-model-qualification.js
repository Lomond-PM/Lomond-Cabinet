...
async function run() {
    ...
    check(firstCapture.textOnly.promptSha256 === "cc9aa49f440748db2fc08d900b5c5ad1fdd6fd75f6d79aab9139e26d16450476" && firstCapture.textOnly.responseFormatSha256 === "85813dd8950079ab9c9542612aa0ad14b82c98e3f3e71f3a370561669e64cdf8" && firstCapture.textOnly.stableRequestBodySha256 === "208e84b1898f38b98f9a16785ab0a10e6c200551d0193b5b0037f968385a3d54" && firstCapture.explicitEditEligible.promptSha256 === "32d55e4db60f7273c00c51004338e59dca14565643561b20420484b9ccd1bb69" && firstCapture.explicitEditEligible.responseFormatSha256 === "509230d09996e81eb3d4baddd332f3730707badd37d6b4d28b4499b6e6ca6b2f" && firstCapture.explicitEditEligible.stableRequestBodySha256 === "953962fb5b390831287a05b2d72811c6f2d474016766dba40209b8aceb5f4a83", "Production Adapter capture reproduces all six frozen C4 Profile SHA values.");
    ...
    check(Object.keys(profileMetadata).join(",") === metadataKeys.join(",") && deeplyFrozen(profileMetadata) && profileMetadata.metadataRevision === "vela-provider-model-qualification-metadata-c4-v1" && profileMetadata.caseProfileFingerprint === c4Fingerprint && profileMetadata.profileFixtureSha256 === crypto.createHash("sha256").update(profileFixtureBytes).digest("hex") && profileMetadata.builderRevision === "vela-capability-prompt-builder-v3" && profileMetadata.requestBranchPolicyRevision === "vela-provider-request-branch-policy-v1" && profileMetadata.capabilityId === "set-opacity-v1" && profileMetadata.capabilityRevision === "vela-capability-contract-v1" && profileMetadata.protocolVersion === "vela.model-response.v1" && profileMetadata.caseCount === 12 && profileMetadata.runsPerCase === 5, "C4 metadata has the exact deep-frozen revision, fixture-byte binding, production revisions, and matrix dimensions.");
    ...
    check(matrixCalls === 12 && completedRun.exitCode === 0 && completedRun.run.schemaRevision === "vela-provider-model-qualification-v3" && completedRun.run.metadata.metadataRevision === "vela-provider-model-qualification-metadata-c4-v1" && completedRun.run.records.every((record, index) => record.requestProfile === qualification.PROFILE_CASES[index].requestProfile) && deeplyFrozen(completedRun.run), "C4 Runner writes a deep-frozen v3 evidence object from C4 metadata and PROFILE_CASES.");
    ...
    const fixtureDriftPath = path.join(fixtureDriftRoot, "provider-branch-profiles-v1.json");
