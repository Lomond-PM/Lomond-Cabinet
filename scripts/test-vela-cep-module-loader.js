...
async function run() {
    ...
    check(result.modules.length === 10 && result.modules[result.modules.length - 1] === "VelaRuntime" && result.modules[result.modules.length - 2] === "VelaController" && result.modules[result.modules.length - 3] === "VelaExecutionAdapter", "Loader returns bounded dependency order.");
    ...
    check(browser.getAppendCount() === 10, "Loader injects each protected module exactly once.");
