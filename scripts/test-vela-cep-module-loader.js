...
async function run() {
    ...
    check(result.modules.length === 14 && result.modules[result.modules.length - 1] === "VelaRuntime" && result.modules[result.modules.length - 2] === "VelaProviderController" && result.modules[result.modules.length - 3] === "VelaController", "Loader returns bounded dependency order.");
    ...
    check(browser.getAppendCount() === 14, "Loader injects each protected module exactly once.");
