...
async function run() {
    ...
    check(JSON.stringify(browser.requestedUrls) === JSON.stringify([
        ...
        "file:///C:/extension/client/js/vela/velaConfirmedAuthorityComposer.js?v=test",
    ...
    const missingComposer = makeBrowser({ failFile: "velaConfirmedAuthorityComposer.js" });
    await expectCode(missingComposer.context.VelaCepModuleLoader.load(), "RUNTIME_CAPABILITY_UNAVAILABLE", "A missing ConfirmedAuthorityComposer fails closed before Runtime.");
    check(missingComposer.context.VelaRuntime === undefined, "Missing E1 Composer cannot produce a partial Runtime.");
