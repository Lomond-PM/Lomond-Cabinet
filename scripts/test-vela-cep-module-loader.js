...
async function run() {
    ...
    check(result.modules.length === 17 && result.modules[2] === "VelaCapabilityContracts" && result.modules[4] === "VelaProviderIntentGate" && result.modules[result.modules.length - 1] === "VelaRuntime" && result.modules[result.modules.length - 2] === "VelaProviderProposalRouter" && result.modules[result.modules.length - 3] === "VelaProviderController", "Loader returns bounded dependency order.");
    ...
    check(browser.getAppendCount() === 17, "Loader injects each protected module exactly once.");
