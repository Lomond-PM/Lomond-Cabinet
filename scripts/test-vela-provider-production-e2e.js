...
function makeHarness() {
    ...
    const environment = Object.assign({}, nodeRuntime, {
        ...
        fetch(url, options) {
            ...
            const requestId = (body.response_format ? /Use requestId (req_[a-z0-9]+)/.exec(JSON.parse(body.messages[1].content).turnResponseContract)[1] : "req_" + "0".repeat(32));
            let envelope;
            ...
            else if (state.providerMode === "logical") envelope = { type: "logicalPlanProposal", steps: [{ capabilityId: "set-opacity-v1", params: { opacity: state.proposalOpacity } }, { capabilityId: "set-layer-name-v1", params: { name: state.proposalName } }] };
            ...
            const response = JSON.stringify({ id: "local", object: "chat.completion", created: 1, model: "m", choices: [{ index: 0, message: { role: "assistant", content: fixtureContent(body, { protocol: "vela.model-response.v1", schemaVersion: "1.1", requestId, provider: "lmstudio", model: "m", envelope }), reasoning_content: state.reasoningContent, tool_calls: [] }, finish_reason: "stop" }], usage: { completion_tokens_details: { reasoning_tokens: state.reasoningTokens } } });
            let sent = false;
    ...
    const runtime = runtimeModule.createRuntime({ activationPolicy, presentationStreaming: false, environment, exactAgentSession: session, invokeHost(source, callback) {
...
function verifyInput(barrier, changes = {}) { const i = barrier.input; return { objectiveId: i.objectiveId, taskId: i.taskId, sessionId: i.sessionId, turnId: i.turnId, taskPlanId: i.taskPlanId, taskPlanRevision: i.taskPlanRevision, stepId: i.stepId, expectedOpacity: 47, ...changes }; }
...
async function run() {
    ...
    const wrongVerifyIdentity = await firstCorrelation.port.verifyCommittedAction(verifyInput(firstCorrelation, { objectiveId: "objective_wrong_verify" }));
    ...
    const wrongVerifyTask = await firstCorrelation.port.verifyCommittedAction(verifyInput(firstCorrelation, { taskId: "task_wrong_verify" }));
    const wrongVerifyExpectation = await firstCorrelation.port.verifyCommittedAction(verifyInput(firstCorrelation, { expectedOpacity: 48 }));
    ...
    for (const field of ["sessionId", "turnId", "taskPlanId", "taskPlanRevision", "stepId"]) {
        const bad = verifyInput(firstCorrelation); bad[field] = field === "taskPlanRevision" ? bad[field] + 1 : "wrong_" + field;
        const rejected = await firstCorrelation.port.verifyCommittedAction(bad);
        check(rejected.state === "blocked" && correlationHarness.calls.filter(call => call.request && call.request.operation === "observeCommittedPropertyValue").length === committedReadsBeforeIdentityProbe, "C2 mismatched " + field + " cannot consume another attempt association");
    }
    const firstVerified = await firstCorrelation.port.verifyCommittedAction(verifyInput(firstCorrelation, { expectedOpacity: 47 }));
    ...
    const duplicateVerify = await firstCorrelation.port.verifyCommittedAction(verifyInput(firstCorrelation, { expectedOpacity: 47 }));
    ...
    check(secondContinuation.state === "verification-required" && correlationHarness.providerBodies.length === providerCountBeforeContinuation + 1 && correlationHarness.calls.filter((call) => call.kind === "execution").length === 1, "valid fresh identity whose target value is already satisfied skips a duplicate Host mutation and still requires fresh verification.");
    correlationHarness.state.nativeLayerId = 99;
    const selectionDriftVerified = await secondCorrelation.port.verifyCommittedAction(verifyInput(secondCorrelation, { expectedOpacity: 47 }));
    ...
    check((await mismatchBarrier.port.verifyCommittedAction(verifyInput(mismatchBarrier, { expectedOpacity: 47 }))).state === "unverified" && mismatchHarness.calls.filter((call) => call.kind === "execution").length === 1, "external post-commit target change maps to unverified without mutation retry.");
    ...
    check((await missingTargetBarrier.port.verifyCommittedAction(verifyInput(missingTargetBarrier, { expectedOpacity: 47 }))).code === "VERIFICATION_UNAVAILABLE", "missing committed target blocks with canonical verification unavailable and no fallback.");
    ...
    const lateVerifyPromise = lateVerifyBarrier.port.verifyCommittedAction(verifyInput(lateVerifyBarrier, { expectedOpacity: 47 }));
    ...
    check((await lateVerifyPromise).state === "cancelled" && (await lateVerifyBarrier.port.verifyCommittedAction(verifyInput(lateVerifyBarrier, { expectedOpacity: 47 }))).state === "blocked", "cancelled in-flight Verify ignores late success and cannot reacquire the consumed capability.");
    ...
    async function lateVerifyLifecycle(label, lifecycle, lateMode) {
        ...
        const pending = barrier.port.verifyCommittedAction(verifyInput(barrier, { expectedOpacity: 47 }));
        ...
        check((await barrier.port.verifyCommittedAction(verifyInput(barrier, { expectedOpacity: 47 }))).state === "blocked", label + " leaves the consumed association unavailable.");
    }
    ...
    async function lateExecutionLifecycle(label, lifecycle, mutationCommitted) {
        ...
        check((await barrier.port.verifyCommittedAction(verifyInput(barrier, { expectedOpacity: 47 }))).state === "blocked", label + " cannot reuse an old verification association.");
        ...
        if (lifecycle !== "dispose") {
            ...
            const freshVerification = await fresh.port.verifyCommittedAction(verifyInput(fresh, { expectedOpacity: 47 }));
            ...
            const expectedExecutions = mutationCommitted === true ? 1 : 2;
            check(harness.calls.filter((call) => call.kind === "execution").length === expectedExecutions && harness.calls.filter((call) => call.request && call.request.operation === "observeCommittedPropertyValue").length === 1, label + " isolates the fresh objective to a mutation only when late Host truth did not already satisfy it, plus one fresh Verify.");
        }
    }
    ...
    async function callbackBeforeSettlementLifecycle(label, lifecycle) {
        ...
        check((await barrier.port.verifyCommittedAction(verifyInput(barrier, { expectedOpacity: 47 }))).state === "blocked", label + " leaves no orphan committed verification entry.");
    }
    ...
    check((await committedUnavailable.barrier.port.verifyCommittedAction(verifyInput(committedUnavailable.barrier, { expectedOpacity: 47 }))).state === "verified", "committed:true plus unavailable execution result can complete through independent committed-target verification.");
    ...
    check(sequence.state.value === 47 && sequence.calls.filter((call) => call.kind === "execution").length === 2 && sequence.calls.filter((call) => call.request && call.request.operation === "observeCommittedPropertyValue").length === 2 && sequence.runtime.getAuthorityProjection().remainingActions === 0, "A2 approve performs one Host mutation and one exact committed-target observation without delegated fallback.");
    ...
    check(sequenceThird.terminal.outcome === "completed" && sequenceThird.objectiveId !== sequenceSecond.objectiveId && sequence.state.value === 47 && sequence.calls.filter((call) => call.kind === "execution").length === 2 && sequence.runtime.getAuthorityProjection().remainingActions === 0, "a new grant permits a fresh third objective, consumes authority normally, and completes an already-satisfied value without another Host mutation.");
    ...
    const staleOutcome = await stalePort.submitIntent({ sessionId: stale.session.getSessionId(), taskId: "stale_task", taskPlanId: "stale_plan", stepId: "stale_step", capabilityIntent: staleIntent });
    check(staleOutcome.state === "blocked" && staleOutcome.code === "CONTEXT_STALE" && staleOutcome.committed === false, "JIT selection drift reports known non-execution before delegated mutation");
    ...
    const postOutcome = await postPort.submitIntent({ sessionId: postcommit.session.getSessionId(), taskId: "post_task", taskPlanId: "post_plan", stepId: "post_step", capabilityIntent: postIntent });
    check(postOutcome.state === "blocked" && postOutcome.code === "PLAN_FAILED" && postOutcome.committed === null, "Host failure retains uncertain commit; consumed authority is not write evidence.");
    ...
    check(rejectedGreeting.state === "completed" && rejectedGreeting.text.includes("localProposal") && rejectedGreeting.proposalCapabilityId === null && greeting.runtime.getProviderUiState().state !== "proposal-ready" && greeting.runtime.getProviderUiState().proposalCapabilityId === null && greeting.runtime.getUiState().candidateId === null, "An ordinary greeting receiving a structured localProposal remains inert assistant prose before Intent Gate and cannot create proposal or candidate authority.");
    await expectCode(greeting.runtime.reviewProviderProposal(), "CANDIDATE_NOT_FOUND", "A plain-chat profile mismatch has no active proposal to review.");
    ...
    check(h.runtime.getConfirmationSurfaceState().revision === 0 && h.runtime.getConfirmationSurfaceState().canApprove === true, "Compatibility first plan revision zero is retained and approvable with captured target.");
    ...
    for (const fact of [false, null, true]) {
        const diagnostic = makeHarness(); await diagnostic.runtime.initialize(); await diagnostic.runtime.grantNextOpacityMutation();
        if (fact !== false) { diagnostic.state.executionError = "HOST_EXECUTION_MUTATION_FAILED"; diagnostic.state.executionMutationCommitted = fact; }
        if (fact === false) await sendProposal(diagnostic, 25);
        else await expectCode(sendProposal(diagnostic, 60), "PLAN_FAILED", "Compatibility route preserves error settlement with explicit commit fact");
        global.AETOOLBOX_DEBUG_REGISTRY = true;
        try { check(diagnostic.runtime.getAuthorityDiagnostics().latestExecution.committed === fact, "C2 compatibility diagnostics use actual tri-state, never consumed grant budget: " + fact); } finally { delete global.AETOOLBOX_DEBUG_REGISTRY; diagnostic.runtime.dispose(); diagnostic.session.close(); }
    }
    ...
    const modelError = makeHarness(); await modelError.runtime.initialize(); modelError.state.providerMode = "error"; const modelErrorResult = await modelError.runtime.sendProviderMessage({ message: "Set opacity to 50%", endpoint: "http://127.0.0.1:1234/v1/chat/completions", model: "m" }); check(modelErrorResult.state === "failed" && modelErrorResult.errorCode === "PROVIDER_RESPONSE_INVALID" && modelError.runtime.getProviderUiState().proposalCapabilityId === null && !JSON.stringify(modelErrorResult).includes("EXPRESSION_NOT_ALLOWLISTED"), "A model-authored error is rejected as a generic local invalid response without proposal state or model error leakage.");
    ...
    for (const opacity of [-1, 100.01]) { const invalid = makeHarness(); await invalid.runtime.initialize(); invalid.state.providerMode = "proposal"; invalid.state.proposalOpacity = opacity; const invalidResult = await invalid.runtime.sendProviderMessage({ message: "Set opacity to 50%", endpoint: "http://127.0.0.1:1234/v1/chat/completions", model: "m" }); check(invalidResult.state === "failed" && invalidResult.errorCode === "PARAM_OUT_OF_RANGE" && invalid.calls.filter((call) => call.kind === "execution").length === 0, "Canonical production parsing rejects delegated opacity " + opacity + " before authority or Host execution."); }
    ...
    check(logicalStep0.state === "awaiting-review" && logicalStep0.logicalPlan.currentStepIndex === 0 && logicalStep0.suspendedReview.capabilityId === "set-opacity-v1" && logicalStep0.suspendedReview.beforeValue === 47 && logicalStep0.suspendedReview.params.opacity === 47 && logicalHappy.providerBodies.length === 1 && logicalHappy.calls.filter((call) => call.kind === "execution").length === 0, "Equal-value production Provider message remains admitted, Driver owns the plan, and exact step 0 reaches Review without mutation or step 1.");
    const logicalStep1 = await logicalHappyDriver.driver.resolveReview({ reviewId: logicalStep0.suspendedReview.reviewId, revision: logicalStep0.suspendedReview.revision, outcome: "approved" });
    check(logicalStep1.state === "awaiting-review" && logicalStep1.logicalPlan.currentStepIndex === 1 && logicalStep1.logicalPlan.completedStepCount === 1 && logicalStep1.suspendedReview.capabilityId === "set-layer-name-v1" && logicalHappy.state.value === 47 && logicalHappy.calls.filter((call) => call.kind === "execution").length === 0 && logicalHappy.calls.filter((call) => call.request && call.request.operation === "observeCommittedPropertyValue").length === 1, "Verified no-op step 0 performs no Host mutation, performs one fresh Verify, advances once, and independently materializes rename Review.");
    const logicalDone = await logicalHappyDriver.driver.resolveReview({ reviewId: logicalStep1.suspendedReview.reviewId, revision: logicalStep1.suspendedReview.revision, outcome: "approved" });
    check(logicalDone.terminal.outcome === "completed" && logicalDone.logicalPlan.completedStepCount === 2 && logicalDone.logicalPlan.partialCompletion === false && logicalHappy.state.name === "Hero" && logicalHappy.calls.filter((call) => call.kind === "execution").length === 1 && logicalHappyDriver.events.filter((event) => event.kind === "task/review-required").length === 2, "A no-op first step and mutating second step complete with one Host mutation, two independent Reviews, and correct cursor progress.");
    const f7 = makeHarness(); f7.state.providerMode = "logical"; f7.state.value = 100; f7.state.proposalOpacity = 60; f7.state.proposalName = "Vela Stream Test"; await f7.runtime.initialize();
    const f7Driver = renameDriver(f7);
    check(f7.runtime.attachObjectiveReviewPort(Object.freeze({
        getProjection() {
            const snapshot = f7Driver.driver.getSnapshot(); const review = snapshot.suspendedReview;
            return snapshot.state === "awaiting-review" && review ? Object.freeze({ state: "active", reviewId: review.reviewId, revision: review.revision, target: review.reviewTarget, stepNumber: snapshot.logicalPlan ? snapshot.logicalPlan.currentStepIndex + 1 : 1, stepCount: snapshot.logicalPlan ? snapshot.logicalPlan.stepCount : 1, capabilityId: review.capabilityId, valueKind: review.capabilityId === "set-layer-name-v1" ? "string" : "number", beforeValue: review.beforeValue, proposedValue: review.capabilityId === "set-layer-name-v1" ? review.params.name : review.params.opacity, outcome: null }) : Object.freeze({ state: "inactive", reviewId: null, revision: null, capabilityId: null, valueKind: null, beforeValue: null, proposedValue: null, outcome: null });
        },
        resolve(input) { return f7Driver.driver.resolveReview(input); }
    })), "F7 attaches the bounded objective Review projection used by the production Surface.");
    const f7Step0 = await f7Driver.driver.startObjective({ message: "把当前图层的不透明度改成 60%，然后把它重命名为 Vela Stream Test", endpoint: "http://127.0.0.1:1234", model: "m" });
    const f7Projection0 = f7.runtime.getConfirmationSurfaceState();
    check(f7Step0.logicalPlan.stepCount === 2 && f7Step0.logicalPlan.currentStepIndex === 0 && f7Step0.suspendedReview.capabilityId === "set-opacity-v1" && f7Step0.suspendedReview.params.opacity === 60 && f7Projection0.capabilityId === "set-opacity-v1" && f7Projection0.valueKind === "number" && f7Projection0.proposedValue === 60, "F7 step 0 materializes and projects only opacity 60.");
    const f7Step1 = await f7Driver.driver.resolveReview({ reviewId: f7Step0.suspendedReview.reviewId, revision: f7Step0.suspendedReview.revision, outcome: "approved" });
    const f7Projection1 = f7.runtime.getConfirmationSurfaceState();
    const f7OpacityExecutions = f7.calls.filter((call) => call.kind === "execution" && call.request.capabilityId === "set-opacity-v1");
    check(f7.state.value === 60 && f7OpacityExecutions.length === 1 && f7Step1.logicalPlan.currentStepIndex === 1 && f7Step1.logicalPlan.completedStepCount === 1 && f7Step1.suspendedReview.capabilityId === "set-layer-name-v1" && f7Step1.suspendedReview.params.name === "Vela Stream Test", "F7 step 0 executes and verifies once, advances once, and materializes exact step 1 rename semantics.");
    check(f7Projection1.capabilityId === "set-layer-name-v1" && f7Projection1.valueKind === "string" && f7Projection1.proposedValue === "Vela Stream Test" && !Object.prototype.hasOwnProperty.call(f7Projection1, "suggestedOpacity"), "F7 step 1 public Review projection is typed rename data with no stale opacity projection.");
    const f7Done = await f7Driver.driver.resolveReview({ reviewId: f7Step1.suspendedReview.reviewId, revision: f7Step1.suspendedReview.revision, outcome: "approved" });
    const f7RenameExecutions = f7.calls.filter((call) => call.kind === "execution" && call.request.capabilityId === "set-layer-name-v1");
    const f7NameVerifications = f7.calls.filter((call) => call.request && call.request.operation === "observeCommittedLayerAttributeValue");
    check(f7RenameExecutions.length === 1 && f7RenameExecutions[0].request.scope.params.name === "Vela Stream Test" && f7NameVerifications.length === 1 && f7.state.name === "Vela Stream Test", "F7 step 1 executes one exact Host rename and verifies the committed layer name once.");
    check(f7.calls.filter((call) => call.kind === "execution" && call.request.capabilityId === "set-opacity-v1").length === 1 && f7Done.terminal.outcome === "completed" && f7Done.logicalPlan.currentStepIndex === 1 && f7Done.logicalPlan.completedStepCount === 2 && f7Done.logicalPlan.remainingStepCount === 0, "F7 never replays opacity and completes the two-step logical cursor.");
    const reverseNoOp = makeHarness(); reverseNoOp.state.providerMode = "logical"; reverseNoOp.state.value = 100; reverseNoOp.state.name = "Vela Stream Test"; reverseNoOp.state.proposalOpacity = 60; reverseNoOp.state.proposalName = "Vela Stream Test"; await reverseNoOp.runtime.initialize();
    const reverseNoOpDriver = renameDriver(reverseNoOp);
    const reverseStep0 = await reverseNoOpDriver.driver.startObjective({ message: "把当前图层的不透明度改成 60%，然后把它重命名为 Vela Stream Test", endpoint: "http://127.0.0.1:1234", model: "m" });
    const reverseStep1 = await reverseNoOpDriver.driver.resolveReview({ reviewId: reverseStep0.suspendedReview.reviewId, revision: reverseStep0.suspendedReview.revision, outcome: "approved" });
    check(reverseStep1.state === "awaiting-review" && reverseStep1.logicalPlan.currentStepIndex === 1 && reverseStep1.logicalPlan.completedStepCount === 1 && reverseNoOp.state.value === 60 && reverseNoOp.calls.filter((call) => call.kind === "execution" && call.request.capabilityId === "set-opacity-v1").length === 1, "A mutating first step executes and advances exactly once to an already-satisfied rename Review.");
    const reverseDone = await reverseNoOpDriver.driver.resolveReview({ reviewId: reverseStep1.suspendedReview.reviewId, revision: reverseStep1.suspendedReview.revision, outcome: "approved" });
    check(reverseDone.terminal.outcome === "completed" && reverseDone.logicalPlan.completedStepCount === 2 && reverseDone.logicalPlan.remainingStepCount === 0 && reverseNoOp.calls.filter((call) => call.kind === "execution" && call.request.capabilityId === "set-layer-name-v1").length === 0 && reverseNoOp.calls.filter((call) => call.request && call.request.operation === "observeCommittedLayerAttributeValue").length === 1 && reverseNoOpDriver.events.filter((event) => event.kind === "task/review-required").length === 2, "An already-satisfied second step skips rename mutation, verifies once, completes the objective, and creates no stale or duplicate Review.");
    const logicalHappyPresentation = presentationModule.create(); logicalHappyPresentation.begin("two-step objective"); logicalHappyPresentation.apply({ state: "pending", text: null, errorCode: null }); const logicalHappyTranscript = logicalHappyPresentation.apply({ state: "completed", text: null, errorCode: null });
    check(logicalHappyTranscript.pending === false && !logicalHappyTranscript.items.some((item) => item.kind === "error" || item.displayTextKey === "vela.surfaceProviderResponse"), "two-step happy terminal consumes structured Provider completion without a false unusable-response error");
    const logicalHundred = makeHarness(); logicalHundred.state.providerMode = "logical"; logicalHundred.state.value = 100; logicalHundred.state.proposalOpacity = 47; logicalHundred.state.proposalName = "Hero"; await logicalHundred.runtime.initialize(); const logicalHundredDriver = renameDriver(logicalHundred); const hundredReview = await logicalHundredDriver.driver.startObjective({ message: "把当前图层透明度改成47%，然后把它命名为Hero", endpoint: "http://127.0.0.1:1234", model: "m" });
    check(hundredReview.state === "awaiting-review" && hundredReview.suspendedReview.beforeValue === 100 && hundredReview.suspendedReview.params.opacity === 47 && hundredReview.logicalPlan.currentStepIndex === 0 && logicalHundred.calls.filter((call) => call.kind === "execution").length === 0, "Opacity 100 follows the same production attachment and reaches step 0 Review before mutation.");
    const logicalPartial = makeHarness(); logicalPartial.state.providerMode = "logical"; logicalPartial.state.proposalOpacity = 47; logicalPartial.state.proposalName = "Hero"; await logicalPartial.runtime.initialize();
    const logicalPartialDriver = renameDriver(logicalPartial); const partial0 = await logicalPartialDriver.driver.startObjective({ message: "把当前图层透明度改成47%，然后把它命名为Hero", endpoint: "http://127.0.0.1:1234", model: "m" }); const partial1 = await logicalPartialDriver.driver.resolveReview({ reviewId: partial0.suspendedReview.reviewId, revision: partial0.suspendedReview.revision, outcome: "approved" });
    check(logicalPartialDriver.driver.cancel() && logicalPartialDriver.driver.getSnapshot().logicalPlan.completedStepCount === 1 && logicalPartialDriver.driver.getSnapshot().logicalPlan.partialCompletion === true && logicalPartial.state.value === 47 && logicalPartial.state.name === "Layer A" && partial1.suspendedReview.capabilityId === "set-layer-name-v1", "Cancelling step 1 preserves committed step 0 as deterministic partial completion without rollback or second mutation.");
    const logicalReject0 = makeHarness(); logicalReject0.state.providerMode = "logical"; logicalReject0.state.proposalOpacity = 47; logicalReject0.state.proposalName = "Hero"; await logicalReject0.runtime.initialize(); const logicalReject0Driver = renameDriver(logicalReject0); const reject0Review = await logicalReject0Driver.driver.startObjective({ message: "把当前图层透明度改成47%，然后把它命名为Hero", endpoint: "http://127.0.0.1:1234", model: "m" }); const reject0Done = logicalReject0Driver.driver.resolveReview({ reviewId: reject0Review.suspendedReview.reviewId, revision: reject0Review.suspendedReview.revision, outcome: "rejected" });
    const reject0Presentation = presentationModule.create(); reject0Presentation.begin("reject step 0"); reject0Presentation.apply({ state: "pending", text: null, errorCode: null }); reject0Presentation.apply({ state: "completed", text: null, errorCode: null }); const reject0Transcript = reject0Presentation.applyConfirmation({ state: "rejected" });
    check(reject0Done.terminal.outcome === "rejected" && reject0Done.logicalPlan.completedStepCount === 0 && logicalReject0.calls.filter((call) => call.kind === "execution").length === 0 && reject0Transcript.items.filter((item) => item.displayTextKey === "vela.surfaceConfirmationRejected").length === 1 && !reject0Transcript.items.some((item) => item.kind === "error"), "step 0 reject mutates nothing and presents only one rejection notice without a false Provider error");
    ...
    const renameNoOp = makeHarness(); renameNoOp.state.proposalCapability = "set-layer-name-v1"; renameNoOp.state.name = "Vela Stream Test"; renameNoOp.state.proposalName = "Vela Stream Test"; await renameNoOp.runtime.initialize();
    const renameNoOpDriver = renameDriver(renameNoOp);
    const renameNoOpReview = await renameNoOpDriver.driver.startObjective({ message: "将当前图层重命名为 Vela Stream Test", endpoint: "http://127.0.0.1:1234", model: "m" });
    const renameNoOpDone = await renameNoOpDriver.driver.resolveReview({ reviewId: renameNoOpReview.suspendedReview.reviewId, revision: renameNoOpReview.suspendedReview.revision, outcome: "approved" });
    check(renameNoOpDone.state === "terminal" && renameNoOpDone.terminal.outcome === "completed" && renameNoOp.calls.filter((call) => call.kind === "execution").length === 0 && renameNoOp.calls.filter((call) => call.request && call.request.operation === "observeCommittedLayerAttributeValue").length === 1 && renameNoOpDriver.events.filter((event) => event.kind === "task/review-required").length === 1, "Already-satisfied rename preserves Review, skips Host mutation and its Undo group, verifies fresh truth, and completes.");
...
run().catch((error) => { console.error(error && error.stack ? error.stack : JSON.stringify(error)); process.exitCode = 1; });
