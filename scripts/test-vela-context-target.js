...
function run() {
    ...
    check(/HOST_ADAPTER_REVISION\s*=\s*"vela-context-host-v2"/.test(hostSource), "Host context adapter revision must be v2.");
    check(/RUNTIME_REVISION\s*=\s*"vela-host-runtime-v2"/.test(indexSource), "Host root runtime revision must be v2.");
    ...
    check(!/capturePropertyTargets|resolvePropertyTarget|readExpression|expressionDigest/.test(bridgeSource + "\n" + hostSource), "The first target-read round must not implement Tier 3 property or expression capture.");
