...
const composition = read("client/js/vela/velaConversationComposition.js");
...
check(composition.indexOf("candidate.session = candidate.owner.getSessionRuntime()") < composition.indexOf("candidate.runtime = options.createRuntime(candidate.session)") && runtimeInit.includes("createOwner: initializeVelaAgentRuntimeOwner"), "Agent owner supplies the exact Session before Runtime construction");
...
check(composition.indexOf("try { unbind(); }") < composition.indexOf("records.forEach(function (entry) { cleanup(entry.candidate); })"), "shutdown unsubscribes Surface before Agent owner disposal");
check(composition.indexOf("candidate.runtime.dispose()") < composition.indexOf("candidate.owner.dispose()"), "Runtime Authority Plane disposes before Agent Session ownership");
...
check(composition.includes("owner.attachObservationReadPort(runtime.getObservationReadPort())"), "main composition root passes the existing Runtime read-only Context port into the Agent owner");
...
check(factoryGuard !== -1 && ownerInit.includes("ownerOptions.AgentObservationRuntime = window.VelaAgentObservationRuntime;"), "cold Owner stores Observation module factories before Runtime availability is considered");
check(ownerInit.indexOf("velaRuntimeController") === -1, "Observation module factory assignment does not depend on an existing Runtime controller");
check(composition.indexOf("candidate.runtime.initialize()") < composition.indexOf("owner.attachObservationReadPort(runtime.getObservationReadPort())"), "only the concrete Observation read port depends on Runtime availability");
check(composition.includes("owner.attachAgentDriverRuntimePort(runtime.getAgentDriverRuntimePort())"), "main wires the narrow Runtime port into the Owner-held Driver after Runtime initialization");
check(composition.includes("runtime.attachObjectiveReviewPort(owner.getObjectiveReviewPort())"), "main wires the Owner-owned objective review port into Runtime composition.");
...
check(/verifyCommittedAction:\s*verifyCommittedAction/.test(runtimeSource) && /preflight\.verifyCommittedValue/.test(runtimeSource), "production Runtime privately routes the semantic Driver operation to typed committed-target verification.");
check(/function verificationInput\(\)[^\n]*objectiveId: active\.objectiveId[^\n]*sessionId: active\.turn\.sessionId[^\n]*taskPlanRevision: active\.taskPlan\.revision[^\n]*expectedValue:/.test(driverSource) && /runtimePort\.verifyCommittedAction\(verificationInput\(\)\)/.test(driverSource) && !/nativeLayerId|propertyPath/.test(driverSource), "Driver passes only logical identity and typed expectation and owns no committed-target execution identity.");
...
const ownerCommitStart = main.indexOf("velaAgentRuntimeOwner = association ? association.agentOwner : null;");
...
check(shutdown.includes("invalidateVelaConversation()") && composition.indexOf("selectedChanged(null, null)") < composition.indexOf("records.forEach(function (entry) { cleanup(entry.candidate); })"), "shutdown invalidates diagnostic Promise and truth before Owner disposal");
