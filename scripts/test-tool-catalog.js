...
// Production integration keeps projection and event ownership centralized.
{
    ...
    check(/id="openVelaTool"[\s\S]*data-tool="vela"/.test(index) && /tool-app app-card is-disabled/.test(index) && /tools\.moreTools\.title/.test(index), "Vela legacy and disabled More Tools static cards remain intact");
