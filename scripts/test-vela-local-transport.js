...
async function run() {
    ...
    const crossProvider = adapterModule.createLocalOpenAICompatibleProvider({ protocol, transport: { sendJson(input) { return otherTransport.sendJson(input); } }, model: "m", requestProfile: "text-only", responseFormatMode: "json-schema", runtime: providerRuntime() });
    ...
    check(trustedSchema.required.length === 6 && trustedSchema.required.join(",") === "protocol,schemaVersion,requestId,provider,model,envelope" && trustedSchema.additionalProperties === false, "Trusted serialization must preserve every canonical response required field in a closed root.");
    const textEnvelope = trustedSchema.properties.envelope;
    check(textEnvelope.additionalProperties === false && textEnvelope.required.join(",") === "type,text" && JSON.stringify(textEnvelope.properties.type.enum) === "[\"text\"]" && Object.prototype.hasOwnProperty.call(textEnvelope.properties, "text") && !Object.prototype.hasOwnProperty.call(textEnvelope.properties, "proposal"), "The text-only Profile exposes exactly the closed text envelope fields.");
    check(!Object.prototype.hasOwnProperty.call(textEnvelope, "oneOf") && !Object.prototype.hasOwnProperty.call(textEnvelope, "anyOf") && !JSON.stringify(textEnvelope).includes("localProposal"), "The text-only Profile has no union or localProposal fallback.");
    check(trustedSchema.properties.protocol.enum.length === 1 && !/code|stage|retryable|details/.test(JSON.stringify(textEnvelope)), "Trusted serialization must keep canonical metadata while omitting model-visible error structure.");
