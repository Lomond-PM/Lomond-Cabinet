...
async function run() {
    ...
    let objectiveProjection = Object.freeze({ state: "active", reviewId: "runtime_review_1", revision: 1, capabilityId: "set-opacity-v1", proposedValue: 47, outcome: null });
    let objectiveResolveCalls = 0;
    check(controller.attachObjectiveReviewPort(Object.freeze({ getProjection() { return objectiveProjection; }, resolve(input) { objectiveResolveCalls += 1; objectiveProjection = Object.freeze({ state: "resolved", reviewId: input.reviewId, revision: input.revision, capabilityId: null, proposedValue: null, outcome: input.outcome }); return Object.freeze({ state: input.outcome === "approved" ? "awaiting-outcome" : "terminal" }); } })) === true, "Runtime attaches one narrow owner-owned objective review port after initialization.");
    check(controller.getConfirmationSurfaceState().state === "confirmation-ready" && controller.getConfirmationSurfaceState().proposedValue === 47, "Active objective review projects through the existing Confirmation facade.");
    ...
    objectiveProjection = Object.freeze({ state: "inactive", reviewId: null, revision: null, capabilityId: null, proposedValue: null, outcome: null });
    ...
    objectiveProjection = Object.freeze({ state: "active", reviewId: "runtime_review_2", revision: 2, capabilityId: "set-opacity-v1", proposedValue: 48, outcome: null });
