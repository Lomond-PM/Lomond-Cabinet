...
async function run() {
    ...
    check(/code: "CONTEXT_STALE", committed: false, observation: Object\.freeze\(\{ targetAvailable: Boolean\(target\), targetClass: target \? "layer-opacity" : null, observedOpacityDigest: record\.valueDigest \}\)/.test(runtimeSource), "Runtime maps approved precommit CONTEXT_STALE to bounded committed-false observation truth without execution identity leakage.");
    ...
    check(/preflight\.verifyCommittedOpacity\s*\(/.test(runtimeSource) && /verifyOpacity:\s*function|verifyOpacity: function/.test(runtimeSource) && !/verifyCommittedOpacity/.test(require("fs").readFileSync(require.resolve("../client/js/vela/velaAgentDriver"), "utf8")), "A2 routes committed-target verification privately through Runtime while retaining the isolated legacy current-selection port.");
