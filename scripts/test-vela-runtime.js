...
async function run() {
    ...
    check(runtimeSource.indexOf('!activePilot || !proposal') === -1 && runtimeSource.indexOf('taskId: activePilot ? activePilot.taskId : input.taskId') !== -1, "Agent Driver intents reach the real PolicyEngine without requiring an active delegation.");
    check(/decision\.decision === "REVIEW_REQUIRED"[\s\S]{0,180}settleAgentDriverProposal\("completed", null, true\)/.test(runtimeSource) && /decision\.decision !== "ALLOW"[\s\S]{0,180}settleAgentDriverProposal\("failed", "PERMISSION_DENIED", false\)/.test(runtimeSource), "REVIEW_REQUIRED and DENY both terminal-settle Runtime-owned Provider proposal state.");
    check(/function settleDelegatedExecutionFailure\([\s\S]*projection\.remainingActions > 0[\s\S]*authorityState = "active"/.test(runtimeSource), "Runtime centralizes pre-commit authority recovery around canonical remaining grant budget.");
    ...
    check(/continuation\.phase = "executing";\s*continuation\.committed = null;\s*return confirmedAuthorityComposer\.executeConfirmed\(\)/.test(runtimeSource), "Runtime establishes callback ownership before executeConfirmed can settle committed truth.");
    check(/composition\.state !== "authority-ready"[\s\S]*confirmedAuthorityComposer\.executeConfirmed/.test(runtimeSource) && /committed === true \|\| satisfied[\s\S]*record\.verificationPlanId[\s\S]*settleProductionExecution\(continuation, execution, "verification-required"\)/.test(runtimeSource), "Production continuation executes only after authority-ready and routes committed or already-satisfied truth through fresh verification.");
    ...
    const registeredTool = Object.freeze({ id: "vela", actions: Object.freeze({ "set-opacity-v1": Object.freeze({ id: "set-opacity-v1" }), "set-layer-name-v1": Object.freeze({ id: "set-layer-name-v1" }) }) });
    ...
    check(Object.isFrozen(validMappings) && validMappings.length === 2 && validMappings.every((mapping) => Object.isFrozen(mapping) && Object.isFrozen(mapping.registeredAction) && mapping.registeredAction.toolId === "vela"), "Runtime startup cross-validation accepts exactly both registered composite action identities and returns only frozen mapping data.");
    ...
    check(Object.keys(controller).sort().join(",") === "approveActiveCandidate,attachObjectiveReviewPort,cancelProviderRequest,checkProviderReadiness,dispose,getAgentDriverRuntimePort,getAuthorityDiagnostics,getAuthorityProjection,getConfirmationSurfaceState,getObservationReadPort,getProviderDiagnostics,getProviderSelectionEvidence,getProviderSurfaceState,getProviderUiState,getStatus,getUiState,grantNextOpacityMutation,initialize,rejectActiveCandidate,resetSession,resume,reviewProviderProposal,revokeOpacityDelegation,sendProviderMessage,subscribePresentationEvents,suspend", "Runtime exposes existing facades, presentation subscription and the read-only selection evidence seam.");
    ...
    let objectiveProjection = Object.freeze({ state: "active", reviewId: "runtime_review_1", revision: 1, target: {compId: "ae-project-1-item-1", layerId: "ae-project-1-item-1-layer-2", revision: 1}, stepNumber: 1, stepCount: 1, capabilityId: "set-opacity-v1", beforeValue: 100, proposedValue: 47, outcome: null });
