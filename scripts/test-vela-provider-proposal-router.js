...
function createHarness(options) {
    ...
    const preflight = { createBoundPlan(input) { creates += 1; check(Object.keys(input).join(",") === "localProposal,selectionOrderMeaningful", "Router supplies only the bounded local proposal shape to the local controller."); check(input.localProposal.capabilityId === "set-opacity-v1" && input.localProposal.params.opacity === 57.5, "Router forwards the normalized opacity exactly once."); return options.createFailure ? Promise.reject(new p.VelaProtocolError(p.ERROR_CODES.CONTEXT_STALE)) : Promise.resolve({ planId: "plan_local", candidateIds: ["cand_local"], review: { valueKind: "number", beforeValue: 20 } }); }, discardBoundPlan() {}, confirmBoundPlan() { return Promise.resolve(); }, executeStep() { return Promise.resolve(); } };
    const controller = controllerModule.createController({ protocol: p, preflight, contextBridge: bridge, reviewPort });
...
async function run() {
    ...
    check(h.getCreates() === 1 && h.provider.getUiState().state === "idle", "Review consumes the proposal exactly once before candidate creation.");
