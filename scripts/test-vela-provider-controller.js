...
async function run() {
    ...
    check(outputVariants[0].properties.text.maxLength === 1024 && outputVariants[1].properties.error.properties.stage.maxLength === 128 && outputVariants[1].properties.error.properties.message.maxLength === 512 && outputVariants[2].properties.proposal.properties.params.properties.opacity.maximum === 100, "The final Transport payload must retain conservative generation caps and bounded localProposal input.");
    check(outputSchema.required.length === 6 && outputVariants.length === 3 && requestBody.stream === false && requestBody.model === "m", "The final payload must retain full required fields, all closed envelope variants, stream:false and the configured model.");
    ...
    const proposalState = await controller.send({ message: "proposal", endpoint: "http://127.0.0.1:1234/v1/chat/completions", model: "m" });
