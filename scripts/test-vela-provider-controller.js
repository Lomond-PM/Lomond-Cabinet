...
async function run() {
    ...
    check(outputVariants[0].properties.text.maxLength === 1024 && outputVariants[1].properties.proposal.properties.params.properties.opacity.maximum === 100, "The final Transport payload must retain bounded text and localProposal input.");
    check(outputSchema.required.length === 6 && outputVariants.length === 2 && !JSON.stringify(requestBody.response_format).includes("\"error\"") && requestBody.stream === false && requestBody.model === "m", "The final payload must retain full required fields, only model-authorized envelope variants, stream:false and the configured model.");
