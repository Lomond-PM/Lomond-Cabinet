...
function runtimeDeferredHarness() {
    ...
    function fetch(url, options) { const body = JSON.parse(options.body); transportCalls.push(body); const requestId = /Use requestId (req_[a-z0-9]+)/.exec(JSON.parse(body.messages[1].content).turnResponseContract)[1]; const responseSchema = body.response_format.json_schema.schema.properties; const response = JSON.stringify({ id: "chatcmpl-local", object: "chat.completion", created: 1784797754, model: "m", choices: [{ index: 0, message: { role: "assistant", content: JSON.stringify({ protocol: responseSchema.protocol.enum[0], schemaVersion: responseSchema.schemaVersion.enum[0], requestId, provider: responseSchema.provider.enum[0], model: "m", envelope: { type: "text", text: "safe" } }), reasoning_content: "", tool_calls: [] }, logprobs: null, finish_reason: "stop" }], usage: { prompt_tokens: 239, completion_tokens: 144, total_tokens: 383, completion_tokens_details: { reasoning_tokens: 0 } }, stats: {}, system_fingerprint: "qwen3.5-4b" }); let sent = false; return Promise.resolve({ status: 200, redirected: false, url, headers: { get: () => "application/json" }, body: { getReader() { return { read() { if (sent) return Promise.resolve({ done: true }); sent = true; return Promise.resolve({ done: false, value: new TextEncoder().encode(response) }); }, cancel() {} }; } } }); }
...
async function run() {
    ...
    const transport = transportModule.createLocalTransport({ protocol: p, fetch(url, options) { const requestBody = JSON.parse(options.body); requestUrls.push(url); requestBodies.push(requestBody); const requestId = /Use requestId (req_[a-z0-9]+)/.exec(JSON.parse(requestBody.messages[1].content).turnResponseContract)[1]; const message = requestBody.messages[2].content; const envelope = message === "将当前图层不透明度设为 57.5%" ? { type: "localProposal", proposal: { capabilityId: "set-opacity-v1", params: { opacity: 57.5 } } } : message === "把当前图层透明度改成47%，然后把它命名为Hero" ? { type: "logicalPlanProposal", steps: [{ capabilityId: "set-opacity-v1", params: { opacity: 47 } }, { capabilityId: "set-layer-name-v1", params: { name: "Hero" } }] } : { text: "safe text", type: "text" }; const body = JSON.stringify({ id: "chatcmpl-local", object: "chat.completion", created: 1784797754, model: "m", choices: [{ index: 0, message: { role: "assistant", content: JSON.stringify({ envelope, model: "m", protocol: p.PROTOCOLS.RESPONSE, provider: "lmstudio", requestId, schemaVersion: p.SCHEMA_VERSION }), reasoning_content: "", tool_calls: [] }, logprobs: null, finish_reason: "stop" }], usage: { prompt_tokens: 239, completion_tokens: 144, total_tokens: 383, completion_tokens_details: { reasoning_tokens: 0 } }, stats: {}, system_fingerprint: "qwen3.5-4b" }); let done = false; return Promise.resolve({ status: 200, redirected: false, url, headers: { get: () => "application/json" }, body: { getReader() { return { read() { if (done) return Promise.resolve({ done: true }); done = true; return Promise.resolve({ done: false, value: new TextEncoder().encode(body) }); }, cancel() {} }; } } }); }, TextDecoder });
    const controller = controllerModule.createProviderController({ protocol: p, contextBridge: bridge, transport, runtime: { setTimeout, clearTimeout, createAbortController() { return { signal: {}, abort() {} }; }, parseUrl(value) { const u = new URL(value); return { protocol: u.protocol, hostname: u.hostname, port: u.port, pathname: u.pathname, username: u.username, password: u.password, search: u.search, hash: u.hash, href: u.href }; }, nowMs: () => 100 } });
