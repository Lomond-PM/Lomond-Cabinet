...
try {
    ...
    assert(/\.primary-action\s*\{[^}]*color:\s*var\(--text-on-accent\);/.test(style), ".primary-action must consume the on-accent text token.");
    assert(/\.palette-editor-action-bar \.palette-library-action\.is-primary\s*\{[^}]*color:\s*var\(--text-on-accent\);/.test(style), "The Palette primary action must consume the on-accent text token.");
    ...
    assert(/\.registry-option-card\s*\{[\s\S]*?border:\s*1px solid var\(--panel-border\)/.test(style) && /\.registry-option-card\.is-active\s*\{[\s\S]*?background:\s*var\(--gold-track\)/.test(style), "Registry tabs must retain the established border and accent tokens.");
