...
try {
    ...
    assert(/--danger-surface:\s*rgba\(255, 107, 95, 0\.22\);/.test(style), "The Action-only danger surface token must preserve its calibrated resting default.");
    assert(/--action-danger-hover-surface:\s*rgba\(255, 107, 95, 0\.30\);/.test(style), "The Danger hover semantic must preserve its calibrated default.");
    ...
    assert(/\.ui-button--primary\s*\{[^}]*color:\s*var\(--action-primary-foreground\);/.test(style), ".ui-button--primary must consume the primary-action foreground semantic.");
    assert(!/\.palette-editor-action-bar \.palette-library-action\.is-primary\s*\{[^}]*color:/.test(style), "The Palette primary action must inherit the canonical CoreUI foreground semantic.");
